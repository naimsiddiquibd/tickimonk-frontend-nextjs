...
// import Waythin from '../../public/waythin.png';
...
const SidebarRight = () => {
    ...
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const [ticketsCount, setTicketsCount] = useState({}); // Object to store ticket counts by event ID
            ...
            try {
                ...
                await Promise.all(
                    eventsData.map(async (event) => {
                        ...
                        if (ticketsByEvent.success) {
                            ticketsCountMap[event._id] = ticketsByEvent.data.length; // Store the count of tickets
                        } else {
                            ticketsCountMap[event._id] = 0; // Default to 0 if there is an error
                        }
                    })
                );
